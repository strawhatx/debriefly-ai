...
const TradeSidebar = () => {
  ...
  useEffect(() => {
    const fetchTrades = async () => {
      ...
      const { data, error } = await supabase.from("positions").select("id,symbol,position_type,pnl,fees,quantity,fill_price,stop_price,entry_date")
      ...
      // IF there is no selected trade set the inital trade
      if (!selectedTrade) setTrade(trades[0])
    };
      ...
      <SidebarContent>
        <div className="p-4">
          ...
          <div className="p-4 border-b border-gray-700 flex justify-between items-center">
            ...
            <Button variant="link" className="p-2 hover:bg-gray-700 rounded-lg">
            ...
            <SidebarMenu className="divide-y-2 border-gray-300">
              {trades.map((trade) => (
                <SidebarMenuItem key={trade.id}>
                  <SidebarMenuButton asChild onClick={() => setTrade(trade)} className={`hover:bg-gray-700 ${trade.id === selectedTrade?.id ? 'bg-gray-700' : ''}`}>
                    <div className="flex flex-col h-fit">
                      <div className="flex justify-between items-start w-full">
                        ...
                        <div className={`text-sm ${trade.position_type === 'LONG' ? 'text-emerald-400' : 'text-red-400'
                          }`}>
                          {trade.position_type === 'LONG' ? (<ArrowUpRight className="w-4 h-4" />)
                      ...
                      <div className="items-left w-full">
                        ...
                        <span className={`font-semibold ${trade.pnl >= 0 ? 'text-emerald-300' : 'text-red-400'}`}>
